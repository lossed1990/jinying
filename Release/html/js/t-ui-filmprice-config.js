var UIFilmPriceConfig = {
    createNew: function() {
        ...
        var m_url_data = "film-morerule-all.zc"; //数据获取接口
        var m_url_add = "film-morerule-add.zc"; //数据新增接口
        var m_url_del = "film-morerule-del.zc"; //数据删除接口
        var m_tablecolumns =   [ { 
            "data": "range"
        },        { 
            "data": "count" 
        ...
        function bingPriceEvent() {
            $m_ui_btn_setprice.on('click', function() {
                if (isNull($m_ui_input_price.val().trim())) {
                    toastr.error('覆膜单价不能为空！');
                    return;
                }

                if (!isPrice($m_ui_input_price.val().trim())) {
                    toastr.error('覆膜单价输入不合法，仅能精确到小数点后两位！');
                    return;
                }
                ...
                $.ajax({    
                    type: "POST",
                    url: "film-setprice.zc",
                    cache:  false,
                    data: JSON.stringify(requestParam),
                    dataType: "json",
                    success: function (result)  {     //封装返回数据    
                        toastr.success('新覆膜单价设置成功');
                        getPrice();
                    },
                    error: function() {
                        toastr.error('新覆膜单价设置失败,请刷新重试！');
                    }  
                });  
            });
        }
...
var g_oFilmPriceConfig = UIFilmPriceConfig.createNew();